...
interface ApiKeySerpProps {
  token: string | null;
}

const API_URL = "https://api.thedataproxy.com/v2/serp";

const ApiKeySerp: React.FC<ApiKeySerpProps> = ({ token }) => {
  const [apiKeys, setApiKeys] = useState<SerpApiKey[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [fullKey, setFullKey] = useState<string | null>(null);
  const [hasSerpApiAccess, setHasSerpApiAccess] = useState<boolean | null>(null);
  ...
  const fetchSerpApiAccess = async () => {
    if (!token) return;
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch SERP API access");
      }
      const data = await response.json();
      setHasSerpApiAccess(data.has_access);
    } catch (err) {
      console.error("Error fetching SERP API access:", err);
      setHasSerpApiAccess(false);
    }
  };
  ...
  const fetchApiKeys = async () => {
    ...
    try {
      ...
      const data: SerpApiKey[] = await response.json();
  ...
  const generateKey = async () => {
    ...
    if (hasSerpApiAccess === null) {
      setError("Checking subscription status...");
      return;
    }
    if (!hasSerpApiAccess) {
      setError("Your subscription plan does not include SERP API features.");
      return;
    }
    setLoading(true);
    setError(null);
      ...
      <Flex direction="column" gap={6}>
        <Box>
          <Flex justify="space-between" align="center" mb={2}>
            <Text fontSize="md" fontWeight="semibold">Create a new API key for SERP access</Text>
        ...
        <Box>
          <Heading size="md" mb={4}>Existing SERP API Keys</Heading>
