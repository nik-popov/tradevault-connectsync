...
function Explore() {
  ...
  const categories = ["all", "owned", ...new Set(scraperAPIs.map((api) => api.category))];
      ...
      <Flex justify="space-between" align="center">
        ...
        <HStack spacing={2}>
          ...
          <Button
            size="sm"
            variant="outline"
            colorScheme={api.isLocked || (!isTrial && !hasSubscription) ? "gray" : "blue"}
            isDisabled={api.isLocked || (!isTrial && !hasSubscription)}
            onClick={() => {
              if (!api.isLocked && (isTrial || hasSubscription)) {
