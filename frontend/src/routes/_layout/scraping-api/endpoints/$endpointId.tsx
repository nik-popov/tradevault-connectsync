// frontend/src/routes/_layout/scraping-api/endpoints/$endpointId.tsx
...
const EndpointDetailPage = () => {
  ...
  const fetchTracerouteData = async () => {
    ...
    try {
      ...
      // Use the current dataset index
      const data = datasets[dataSetIndex];
      setTracerouteData(data);

      // Toggle dataset index for next refresh (0 or 1)
      ...
      component: () => (
        <Box p={4}>
          <Flex justify="space-between" align="center" mb={4}>
            ...
            <Flex gap={2}>
              <Tooltip label="Toggle automatic refresh of traceroute data">
                <Button
                  size="sm"
                  colorScheme={autoRefresh ? "green" : "gray"}
                  onClick={() => setAutoRefresh(!autoRefresh)}
