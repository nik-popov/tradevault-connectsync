  ...
  return (
    <Box p={4} borderWidth={1} borderRadius="md">
      ...
      {isLoading ? (
        ...
        <Alert status="error">
          <AlertIcon />Error loading connections
        </Alert>
      ) : (
        <Table variant="simple">
          <Thead>
  ...
  return (
    <Box p={4} borderWidth={1} borderRadius="md">
      ...
      {isLoading ? (
        ...
        <Alert status="error">
          <AlertIcon />Error loading options
        </Alert>
      ) : (
        <VStack align="stretch">
          {options.map((opt: any) => (
            <Button key={opt.id} colorScheme="blue">
              {opt.name}
            </Button>
          ))}
        </VStack>
...
/** Dummy Route Component for ProxyComponents */
const ProxyComponentsRoute = () => {
  return (
    <Box p={4}>
      <Text fontSize="lg">Proxy Components Route</Text>
      {/* This component can be expanded or replaced with nested routing logic if needed */}
    </Box>
  );
};

/**
 * Export a Route using createFileRoute so that
 * routeTree.gen.ts can import it as expected.
 */
export const Route = createFileRoute("/_layout/proxies/ProxyComponents")({
  component: ProxyComponentsRoute,
});

// Export the individual proxy-related components
